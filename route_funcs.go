	...
	for _, sp := range splitParams {
		if strings.HasSuffix(sp, paramEndStr) && strings.HasPrefix(sp, paramBeginStr) {
			...
			if len(splits) == 2 {
				...
				return "", nil, errors.New("Invalid route parameter setting: " + sp)
			}
			if !checkParamName(paramName) {
				return "", nil, errors.New("Invalid route parameter name '" + paramName + "': " + sp)
			}
			opt := RouteOption{}
			...
			paramPath = paramPath + paramBeginStr + paramName + paramEndStr
		} else {
			paramPath = paramPath + sp
		}
	}
